...
public class HTMLMetaProcessor {
	 ...
	 * based on any META tags found under the given <code>node</code>.
	 * @throws MalformedURLException 
	 */
	public static final void getMetaTags(HTMLMetaTags metaTags, Node node, String currURL) throws MalformedURLException {

		
		metaTags.reset();
		getMetaTagsHelper(metaTags, node, new URL(currURL));
	}
					...
					if (contentNode != null) {
						...
						if ("pragma".equals(name)) {
							...
							if (metaTags.getRefresh() && idx != -1) { // set the URL
								...
								if (idx != -1) {
									...
									try {
										...
										// XXX according to the spec, this has to be an absolute url. However, many websites use relative URLs and expect browsers to handle that.
										// Unfortunately, in some cases this may create a infinitely recursive paths (a crawler trap)...
							...
							if (metaTags.getRefresh()) {
								if (refreshUrl == null) {
									// apparently only refresh time was present. set the URL to the same URL.
									refreshUrl = currURL;
								}
